...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description="Generate a static index page from a template, with JPEGs for the thumbnails."
    )
    argparser.add_argument(
        "--manifest-file", default=MANIFEST_FILE, help="Path to the manifest JSON file"
    )
    argparser.add_argument(
        "--template-file", default=TEMPLATE_FILE, help="Path to the template HTML file"
    )
    argparser.add_argument(
        "--index-file",
        default=INDEX_FILE,
        help="Path to put the generated index HTML file",
    )
    argparser.add_argument(
        "--render-dir", default=RENDER_DIR, help="Path to the render directory"
    )
    argparser.add_argument(
        "--site-root",
        default=SITE_ROOT,
        help="Path for the root of the index page; default is `" + SITE_ROOT + "`",
    )
    argparser.add_argument(
        "--wsgi-url",
        default=None,
        help="Path for WSGI request handler; default is `{site root}/wsgi`",
    )
    argparser.add_argument(
        "--poster-replace-tag",
        default=POSTER_REPLACE_TAG,
        help="The string in the template to be replaced by the poster content",
    )
    argparser.add_argument(
        "--pamphlet-replace-tag",
        default=PAMPHLET_REPLACE_TAG,
        help="The string in the template to be replaced by the pamphlet content",
    )
    argparser.add_argument(
        "--online-replace-tag",
        default=ONLINE_REPLACE_TAG,
        help="The string in the template to be replaced by the online-only content",
    )
    argparser.add_argument(
        "--auth-tag-file-basic",
        ...
        help="The file containing the authorisation text specifying only a town/city (for digital material).",
    )
    argparser.add_argument(
        "--skip-prefix",
        default=SKIP_PREFIX,
        help="Skip items starting with this prefix; default is '"
        + SKIP_PREFIX
        + "'; pass an empty string for no skipping",
    )
    ...
    if not os.path.exists(CONVERT_PATH):
        ...
        if os.name == "posix":
            converttry = subprocess.run(
                ["which", CONVERT], stdout=subprocess.PIPE, universal_newlines=True
            ).stdout.strip()
            if converttry:
    ...
    with open(arguments.manifest_file, "r") as mani_fp:
        ...
        for mkey in sorted(manifest.keys()):
                ...
                for k in mkey.split("/"):
                    if k.startswith(arguments.skip_prefix):
                        printv(
                            "Skipping",
                            mkey,
                            "for filename including the skip prefix",
                            arguments.skip_prefix,
                        )
                        skipme = True
                        break
                if skipme:
            ...
            r_out = os.path.join(
                arguments.render_dir, ourfile[0:-4] + "_preview" + ".jpg"
            )
            ...
            if not (
                ...
                and (os.path.getmtime(r_in) <= os.path.getmtime(r_out))
            ):
                flippy = subprocess.run(
                    [CONVERT_PATH] + preconvargs + [r_in] + convargs + [r_out],
    ...
    with open(arguments.template_file) as templatefp:
        out_str = templatefp.read().replace(
            arguments.poster_replace_tag, poster_replacement_str
        )
        out_str = out_str.replace(
            arguments.pamphlet_replace_tag, pamphlet_replacement_str
        )
    ...
    site_root = (
        "" if (arguments.site_root == ".") else (arguments.site_root.rstrip("/") + "/")
    )
    out_str = out_str.replace("SITE_ROOT", site_root)

    wsgi_url = (
        (site_root + "wsgi") if (arguments.wsgi_url is None) else arguments.wsgi_url
    )
