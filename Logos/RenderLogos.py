...
def main():
    ...
    argparser.add_argument(
        "--page-root",
        default=PAGE_ROOT,
        help="Path for the root of the index page; default is `" + PAGE_ROOT + "`",
    )
    ...
    for bp in BACKEND_PATHS:
        ...
        if os.name == "posix":
            backendtry = subprocess.run(
                ["which", BACKEND], stdout=subprocess.PIPE, universal_newlines=True
            ).stdout.strip()
            if backendtry:
    ...
    for okey in sorted(
        outputs.keys(), key=lambda k: (os.path.dirname(k), os.path.basename(k))
    ):
        item = outputs[okey]
        ...
        caption = os.path.splitext(
            okey.replace(os.path.commonpath([okey, SOURCE_DIR]), "").lstrip("/")
        )[0]
