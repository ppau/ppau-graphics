...
def application(env, start_response):
    ...
    if head[0] == "200 OK":
        ...
        inkv = subprocess.Popen([inky, "-V", "--no-gui"], stdout=subprocess.PIPE).stdout
        inkscape_version = "1"
        ...
        for pn in pagenums:
            ...
            qfile = os.path.join(
                doc_root, page_root, SOURCE_DIR, item[str(pn)][0][:-9] + ".svg"
            )
            print(qfile, pn)
            # sed
            ...
            sed = subprocess.Popen(
                [seddy, "-e", authsub, "-e", printsub, qfile], stdout=subprocess.PIPE
            )
            # inkscape
            inkscape_args = []
            if inkscape_version == "1":
